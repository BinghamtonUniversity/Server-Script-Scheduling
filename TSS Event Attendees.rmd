...
for (event_id in event_ids) {
  ...
  url <- paste0('https://bengaged.binghamton.edu/rss_event_attendees?include_deactivated_users=1&event_id=', event_id)
...
event_attendees <- event_attendees %>%
  left_join(df %>% select(eventId, title, eventDate, description, fullDescription, eventLink), by = "eventId")

# Display the final dataframe with event titles
event_attendees %>% relocate(title, eventDate, description, fullDescription, eventLink)

```

```{r}
write.csv(event_attendees, "Z:/Shared SAASI/Data Hub Development/B-Engaged Migration/B-Engaged Dumps/Attendees/TSS Event Attendees.csv")
```
